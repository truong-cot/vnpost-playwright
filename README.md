...
_Lưu ý: Nếu gặp lỗi trùng cổng (`EADDRINUSE`), hãy chỉ định cổng khác để chạy:_
...
- **Bước 1: Cập nhật Page Object (POM)**:
  ...
  - _Không được_ viết trực tiếp các CSS Selector/XPath phức tạp bên trong file kịch bản kiểm thử (`*.spec.ts`).
...
_Ví dụ chạy testcase `BANGGIA_41`:_
...
## 🛠️ Quy Trình Gỡ Lỗi & Mẫu Kịch Bản Chuẩn (Debugging & Prompt Standards)

### 1. Mẫu Kịch Bản Kiểm Thử Yêu Cầu (Sample Prompt)

Khi gửi yêu cầu viết autotest mới, khuyến khích sử dụng mẫu cấu trúc chuẩn sau:

```text
TESTCASE: [MÃ_TESTCASE]
- Kiểm tra chức năng: [Mô tả tính năng cần kiểm thử]
- Các bước thực hiện:
"1. [Bước 1: Ví dụ đăng nhập]
2. [Bước 2: Ví dụ click chọn sản phẩm]
3. [Bước 3: Ví dụ click nút cập nhật]"
- Kết quả mong muốn: [Mô tả kết quả và xác nhận cụ thể]
```

### 2. Các Bước Xử Lý Khi Gặp Lỗi (Troubleshooting & Debugging)

Khi kịch bản kiểm thử bị thất bại (Fail), hãy áp dụng quy trình xử lý 3 bước:

1. **Phân tích log lỗi chi tiết**: Xem thông tin log lỗi từ terminal hoặc mở báo cáo HTML bằng lệnh `npx playwright show-report --port 9325`.
2. **Khắc phục lỗi định vị (Locators)**:
   - **Lỗi Strict Mode (Nhiều hơn 1 phần tử)**: Dùng `.first()`, `.last()` hoặc chỉ định chính xác option `{ exact: true }` trong `getByRole`.
   - **Lỗi Scoped (Phần tử nằm ngoài container)**: Mở rộng phạm vi tìm kiếm của locator lên `.ant-drawer` hoặc `[role="dialog"]`.
   - **Lỗi Toast tự tắt**: Sử dụng các selector lớp bao quát của Ant Design như `.ant-message-notice` hoặc `.ant-notification-notice` kết hợp `.first()`.
3. **Thực thi lại và xác nhận**: Chạy test ở chế độ headed hoặc sử dụng UI Mode (`--ui`) để quan sát trực tiếp hành động.

### 3. Câu Lệnh Chạy Kiểm Thử Theo Tên (Grep) - Headed Mode

Để chạy riêng biệt một testcase cụ thể ở chế độ hiển thị giao diện trình duyệt Chrome:

```bash
npx playwright test tests/product/detail-product.spec.ts -g "SANPHAM_27" --project=chromium --headed
```
